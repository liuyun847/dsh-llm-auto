 ...
 *
 * ## 它在上层看起来是什么
 * provider `auto` 下唯一一条模型 `auto`:`resolveModel()` 回显 `provider/id` 并给出一个
 * 上下文窗口(具体口径由注入的 `deps.resolveContextWindow` 决定 —— 宿主里默认是
 * "由 `compactWindow` 反算出来的声明窗口",见 lib/compact.js;老口径是取第一条可解析路由的
 * 窗口,取不到用保守值),`listModels()` 给出目录里那一条。
 * 这样模型选择器里就多出一个 `Auto` 分组。
 *
 * ## 它是怎么把请求转给下一条的
 * 对每条候选路由发起一次**嵌套调用** `ctx.llm.stream({ ...options, provider, model })`,
 * 并把分片按下面的纪律转交出去。
 *
 * ## 五条来自真机/源码的硬约束(改代码前先读)
 *
 ...
 *
 * 5. **历史回放:嵌套调用前必须把助手消息的 source 改回真实路由**。`auto` 产出的历史消息
 *    `source.provider` 写的是 `auto`(外层请求的 provider),而 `LlmRuntime.forAdapter` 只保留
 *    "历史 provider 的适配器 === 本次适配器"的 replay 状态 ⇒ 不改写就会被剥掉,pi-ai 会把
 *    思考块摊平进正文、把 `reasoning_content` 填成空串。修正在 `#nestedOptions` 里做,
 *    纯函数在 lib/replay.js。
 *
...
import { restoreReplaySources } from './replay.js'
...
export class AutoAdapter extends LlmAdapter {
  /** @type {{ llm: object, routes: Array<{provider: string, model: string}>, modelName: string|(() => string), retryPolicy?: object, resolveContextWindow: (signal?: AbortSignal) => Promise<number>, ring?: object, logger?: object, sleep?: (ms: number, signal?: AbortSignal) => Promise<boolean>, random?: () => number }} */
  #deps
  /** 模型显示名的取值器(见构造函数的 `deps.modelName` 说明)。 */
  #modelName
  #chainLabel
   ...
   * @param deps.modelName - 模型显示名(config.name);**也可以是取值函数**。宿主里 `config.name`
   *   是 schemastery 的 volatile 引用(设置页可改),传取值函数才能让改动不必重启就生效;
   *   传字符串(单测、直接构造)时按固定值处理。
  ...
  constructor(deps) {
    super()
    this.#deps = deps
    this.#modelName = typeof deps.modelName === 'function' ? deps.modelName : () => deps.modelName
   ...
   * 同时把历史助手消息的 source 改回它 replay 状态记录的真实路由(见 lib/replay.js)。
   *
   ...
   *
   * 来源改写必须在**这里**(换 provider 的那一刻),不能等嵌套调用的 `forAdapter`:
   * 那一刻判的是"历史 provider 的适配器 === 嵌套适配器",而 `auto` 归本插件自己的适配器,
   * 不改写就会被剥掉 replay 状态、思考被摊平进正文(详见 lib/replay.js 的模块注释)。
   * @param options - 调用方的完整请求。
   * @param route - 本次候选路由。
   * @returns 交给 `ctx.llm.stream` 的请求。
   */
  async #nestedOptions(options, route) {
    const sourced = restoreReplaySources(options)
    return pickRouteEffort({ ...sourced, provider: route.provider, model: route.model }, route, this.#deps.llm)
  }
