...
//
// ⚠ 本文件在插件目录与 profile 的 node_modules 各有一份，改完必须同步：
//   其余 lib/*.js 两侧是**硬链接**，本文件曾以独立副本存在（复制而来）。
//   同步做法见 README §4 第 1 条：删 node_modules 侧 + 重建硬链接 + SHA256 复核；
//   会 rename 的编辑器（VS Code 等）保存时会打断链接，需重新建链。
//
