 ...
 * 「provider + model」,某条失败就静默切下一条 —— 对上层(agent loop / 会话日志 /
 * 压缩链路)它就是一个普通模型。
 *
 * ── 它到底怎么工作(一段话) ────────────────────────────────────────────────
 ...
 *
 * ── 生效条件(踩过) ────────────────────────────────────────────────────────
 * `file:` 依赖在 pnpm 下是**实体拷贝**,真正被加载的是
 * `~\.dsh\profiles\<profile>\node_modules\dsh-llm-auto\`;改 `plugins\` 下的源码**不生效**。
 * 改完必须重新同步并让宿主重启(loader 按 URL 缓存已 import 的模块):
 *   node <工作区>\dsh-plugin-manager\dshpm.mjs add \
 *     file:C:\Users\MLTZ\.dsh\profiles\web\plugins\dsh-llm-auto --profile web
 ...
 * 配置一律在 `normalizeRoutes()` 里校验:结构问题打一条 error 并**拒绝注册**(不抛,
 * 遵循本机既有约定 —— 插件配置错误不该让宿主起不来),单条问题 warn 后跳过。
 * 交给 schemastery 会把"少写一个 model 字段"变成整行加载失败,反而更难排查。
 *
 * ── 边界(详见 README) ────────────────────────────────────────────────────
 *  · 只"失败时切换":不记账、不按价格/能力挑路由,顺序完全由 config.routes 决定;
 *  · 已经向调用方交出内容之后的上游失败**不回退**(避免半截回答 + 重新回答);
...
export function apply(ctx, config = {}) {
  ...
  logger.info(`auto: 已注册路由 ${AUTO_PROVIDER}/${AUTO_MODEL}（${modelName}）→ ${describeChain(routes)}`)
